...
export const runForceDirectedGraph = (
  ...
  let links = linksData.map((link) => Object.assign({}, link));
  let nodes = nodesData.map((node) => Object.assign({}, node));
  ...
  const getNodeSize = (node) => {
    ...
    linksData.forEach((link) => {
      if (link.source === node.id || link.target === node.id) count++;
    });
    return count;
  };
  ...
  const updateGraphByHouse = (houseName) => {
    ...
    houseNodes.forEach((node) => {
      ...
      const relatedNodes = filteredLinks.map((d) =>
        d.source !== node.id ? d.source : d.target
      );
      newFilteredNodes.push(
        nodesData
          .filter((d) => d.id === node.id || relatedNodes.indexOf(d.id) >= 0)
          .map((d) => Object.assign({}, d))
      );
    });
    ...
    newFilteredNodes = [
      ...new Map(
        flattedFilteredNodes.map((item) => [item["id"], item])
      ).values(),
    ];
  ...
  // Add a node "All Characters" into the list of options
  nodes.unshift({
    id: "All Characters",
  });
  ...
  d3.select("#selectBaratheon").on("click", () =>
    updateGraphByHouse("Baratheon")
  );
  d3.select("#selectLannister").on("click", () =>
    updateGraphByHouse("Lannister")
  );
  d3.select("#selectTargaryen").on("click", () =>
    updateGraphByHouse("Targaryen")
  );
  ...
  const updateGraphByCharacter = (characterName) => {
    if (characterName !== "All Characters") {
      ...
      const filteredLinks = linksData
        .filter((d) => d.source === characterName || d.target === characterName)
        .map((d) => Object.assign({}, d));
      const relatedNodes = filteredLinks.map((d) =>
        d.source !== characterName ? d.source : d.target
      );
      const filteredNodes = nodesData
        .filter(
          (d) => d.id === characterName || relatedNodes.indexOf(d.id) >= 0
        )
        .map((d) => Object.assign({}, d));
      links = filteredLinks;
      nodes = filteredNodes;
  ...
  const updateGraph = () => {
    ...
    labelElements
      .on("mouseover", (event, d) =>
        addTooltip(renderTooltip, d, event.pageX, event.pageY)
      )
